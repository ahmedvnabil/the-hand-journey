...
const begin = async (input: 'camera' | 'pointer') => {
  ...
  await start(canvasRef.value, containerRef.value, input)
}
...
<template>
  <main ref="containerRef" class="relative h-full w-full touch-none overflow-hidden bg-ink" aria-label="The Hand Journey — an interactive story controlled by your hands">
    ...
    <PermissionGate v-if="stage === 'gate' || stage === 'starting' || stage === 'failed'" :stage="stage" :error="errorMessage" @begin="begin" />
